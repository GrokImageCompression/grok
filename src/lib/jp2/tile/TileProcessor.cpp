...
void TileProcessor::makeLayerFeasible(uint32_t layno, uint16_t thresh,
		bool final) {
...
bool TileProcessor::compressTilePart(	uint32_t *tile_bytes_written) {
	...
	if (m_tilePartIndex == 0 && plt_markers){
...
bool TileProcessor::mct_encode() {
	...
	if (m_tcp->mct == 2) {
		...
		mct::compress_irrev(tile->comps[0].getBuffer()->getHighestBufferResWindowREL()->getBuffer(),
				...
				tile->comps[2].getBuffer()->getHighestBufferResWindowREL()->getBuffer(), samples);
	} else {
...
void TileProcessor::t1_encode() {
	...
	auto scheduler = std::unique_ptr<T1CompressScheduler>(new T1CompressScheduler(tile,needsRateControl()));
...
#endif
	if (!l_t2->compressPackets(m_tileIndex, m_tcp->numlayers, m_stream,
	...
	if (tilePartDataLength) {
		...
		if (bytesLeftInStream == 0){
			GRK_ERROR("Tile %d, tile part %d: stream has been truncated and "
					"there is no tile data available",m_tileIndex,tcp->m_tilePartIndex);
			return false;
		}
