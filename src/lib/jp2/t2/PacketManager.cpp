...
PacketManager::PacketManager(bool compression,
							GrkImage *img,
												  ...
												  tileProcessor(tileProc)
{
	...
	if (compression) {
		...
		updateCompressTcpProgressions(cp,
											image->numcomps,
											tileno,
											tileBounds,
											max_precincts,
...
void PacketManager::enableTilePartGeneration(uint32_t pino,
...
void PacketManager::getParams(const GrkImage *image,
...
void PacketManager::updateCompressTcpProgressions(CodingParams *p_cp,
									uint16_t num_comps,
									uint16_t tileno,
									grkRectU32 tileBounds,
									uint64_t max_precincts,
...
void PacketManager::updateCompressParams(const GrkImage *image,
									CodingParams *p_cp,
									uint16_t tileno) {
	assert(p_cp != nullptr);
	assert(image != nullptr);
	...
	getParams(image,
				...
				&max_res,
				nullptr);
	updateCompressTcpProgressions(p_cp,
						image->numcomps,
						tileno,
						tileBounds,
						max_precincts,
 ...
 */
bool PacketManager::checkForRemainingValidProgression(int32_t prog,
												uint32_t pino,
