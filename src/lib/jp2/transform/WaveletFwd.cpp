...
/* </summary>                            */
template<typename T> void deinterleave_h(const T * GRK_RESTRICT a,
                                   T * GRK_RESTRICT b,
                                   int32_t dn,
                                   int32_t sn, int32_t parity)
{
    int32_t i;
...
#else
void dwt97::encode_step1_combined(float* fw,
									uint32_t iters_c1,
									uint32_t iters_c2,
									const float c1,
									const float c2)
...
void dwt97::encode_step2(float* fl,
							float* fw,
							 uint32_t end,
							 uint32_t m,
							 float c)
...
void dwt97::encode_1_real(float *w,
							int32_t dn,
							int32_t sn,
							int32_t parity)
{
    ...
    encode_step2(w + a, w + b + 1,
                         (uint32_t)dn,
                         (uint32_t)std::min<int32_t>(dn, sn - b),
                         alpha);
    encode_step2(w + b, w + a + 1,
                         (uint32_t)sn,
                         (uint32_t)std::min<int32_t>(sn, dn - a),
                         beta);
    encode_step2(w + a, w + b + 1,
                         (uint32_t)dn,
                         (uint32_t)std::min<int32_t>(dn, sn - b),
...
template <typename T, typename DWT> struct encode_h_job{
	dwt_line<T> h;
...
template <typename T, typename DWT> void encode_h_func(encode_h_job<T,DWT> *job){
    uint32_t j;
    for (j = job->min_j; j < job->max_j; j++) {
        T* GRK_RESTRICT aj = (T*)job->tiledp + j * job->w;
        job->dwt.encode_and_deinterleave_h_one_row((T*)aj,
        										(T*)job->h.mem,
												job->rw,
...
template<typename T, typename DWT> struct encode_v_job{
	dwt_line<T> v;
    uint32_t rh;
    uint32_t w;
    T * GRK_RESTRICT tiledp;
...
template <typename T, typename DWT> void encode_v_func(encode_v_job<T,DWT> *job){
    uint32_t j;
    for (j = job->min_j; j + NB_ELTS_V8 - 1 < job->max_j; j += NB_ELTS_V8) {
        job->dwt.encode_and_deinterleave_v((T*)job->tiledp + j,
                                            (T*)job->v.mem,
                                            job->rh,
                                            job->v.parity == 0,
                                            job->w,
...
template< typename T> void fetch_cols_vertical_pass(const T *array,
										T *tmp,
										uint32_t height,
...
/* columns. */
template<typename T> void deinterleave_v_cols(const T * GRK_RESTRICT src,
								T * GRK_RESTRICT dst,
								uint32_t dn,
								uint32_t sn,
								uint32_t stride_width,
...
void dwt97::grk_v8dwt_encode_step1(float* fw,
                                   uint32_t end,
                                   const float cst){
    uint32_t i;
#ifdef __SSE__
...
void dwt97::grk_v8dwt_encode_step2(float* fl,
								float* fw,
                                   uint32_t end,
                                   uint32_t m,
                                   float cst){
...
/* </summary>                           */
template<typename T, typename DWT> bool WaveletFwdImpl::encode_procedure(TileComponent * tilec){
	...
	uint32_t stride = tilec->getBuffer()->getHighestResWindowREL()->stride;
    T * GRK_RESTRICT tiledp = (T*)tilec->getBuffer()->getHighestResWindowREL()->data;
    ...
    while (i--) {
        ...
        if (num_threads <= 1 || rw < 2 * NB_ELTS_V8) {
            ...
            for (j = 0; j + NB_ELTS_V8 - 1 < rw; j += NB_ELTS_V8) {
                dwt.encode_and_deinterleave_v((T*)tiledp + j,
                                            bj,
                                            rh,
                                            parity_col == 0,
                                            stride,
        ...
        if (num_threads <= 1 || rh <= 1) {
            ...
            for (j = 0; j < rh; j++) {
                ...
                dwt.encode_and_deinterleave_h_one_row(aj, bj, rw,
                                                       parity_row == 0 ? true : false);
            }
        }  else {
            uint32_t num_jobs = (uint32_t)num_threads;
            uint32_t step_j;
...
void dwt53::encode_and_deinterleave_v(int32_t *arrayIn,
										int32_t *tmpIn,
										uint32_t height,
										bool even,
										uint32_t stride_width,
...
#define GRK_Dc(i) tmp[((1+(i)*2))* NB_ELTS_V8 + c]
    ...
    if (cols == NB_ELTS_V8) {
        deinterleave_v_cols(tmp, array, dn, sn,
...
void dwt53::encode_and_deinterleave_h_one_row(int32_t * rowIn,
										int32_t * tmpIn,
										uint32_t width,
										bool even)
{
...
/* where cols <= NB_ELTS_V8 */
void dwt97::encode_and_deinterleave_v(float *arrayIn,
													float *tmpIn,
													uint32_t height,
													bool even,
													uint32_t stride_width,
													uint32_t cols)
{
    ...
    grk_v8dwt_encode_step2(tmp + a * NB_ELTS_V8,
                           tmp + (b + 1) * NB_ELTS_V8,
                           dn,
                           std::min<uint32_t>(dn, sn - b),
                           alpha);
    grk_v8dwt_encode_step2(tmp + b * NB_ELTS_V8,
                           tmp + (a + 1) * NB_ELTS_V8,
                           sn,
                           std::min<uint32_t>(sn, dn - a),
                           beta);
    grk_v8dwt_encode_step2(tmp + a * NB_ELTS_V8,
                           tmp + (b + 1) * NB_ELTS_V8,
                           dn,
    ...
    grk_v8dwt_encode_step2(tmp + b * NB_ELTS_V8,
                           ...
                           std::min<uint32_t>(sn, dn - a),
                           delta);
    grk_v8dwt_encode_step1(tmp + b * NB_ELTS_V8, (uint32_t)dn,
                           grk_K);
    grk_v8dwt_encode_step1(tmp + a * NB_ELTS_V8, (uint32_t)sn,
                           grk_invK);


    if (cols == NB_ELTS_V8) {
        deinterleave_v_cols(tmp,
							array,
							dn, sn,
							stride_width, even ? 0 : 1, NB_ELTS_V8);
    } else {
...
void dwt97::encode_and_deinterleave_h_one_row(float* rowIn,
											float* tmpIn,
											uint32_t width,
											bool even)
{
