...
static void decompress_v_final_memcpy_53(const int32_t* buf, const uint32_t height, int32_t* dest,
										 const size_t strideDest)
{
	for(uint32_t i = 0; i < height; ++i)
	{
		...
		STOREU(&dest[(size_t)i * strideDest + VREG_INT_COUNT],
			   LOAD(&buf[PLL_COLS_53 * i + VREG_INT_COUNT]));
	}
}

/** Vertical inverse 5x3 wavelet transform for 8 columns in SSE2, or
 * 16 in AVX2, when top-most pixel is on even coordinate */
static void decompress_v_cas0_mcols_SSE2_OR_AVX2_53(int32_t* buf, int32_t* bandL, /* even */
													...
													int32_t* dest, const uint32_t strideDest)
{
	...
	if(total_height > 3)
	{
		...
		for(i = 0, j = 1; i < (total_height - 3); i += 2, j++)
		{
			...
			STORE(buf + PLL_COLS_53 * (i + 1) + VREG_INT_COUNT,
	...
	if(total_height & 1)
	{
		...
		STORE(buf + PLL_COLS_53 * (total_height - 2),
		...
		/* d1n + ((s0n + tmp_len_minus_1) >> 1) */
		STORE(buf + PLL_COLS_53 * (total_height - 2) + VREG_INT_COUNT,
...
static void decompress_v_cas1_mcols_SSE2_OR_AVX2_53(int32_t* buf, int32_t* bandL, const uint32_t hL,
													...
													int32_t* dest, const uint32_t strideDest)
{
	...
	/* in_odd[0] - ((in_even[0] + s1 + 2) >> 2); */
	VREG dc_1 = SUB(LOADU(in_odd + VREG_INT_COUNT),
	...
	for(i = 1, j = 1; i < (total_height - 2 - !(total_height & 1)); i += 2, j++)
	{
		...
		VREG dn_1 =
			SUB(LOADU(in_odd + j * strideL + VREG_INT_COUNT), SAR(ADD3(s1_1, s2_1, two), 2));
	...
	if(!(total_height & 1))
	{
		...
		VREG dn_0 = SUB(LOADU(in_odd + (size_t)(total_height / 2 - 1) * strideL),
						SAR(ADD3(s1_0, s1_0, two), 2));
		VREG dn_1 = SUB(LOADU(in_odd + (size_t)(total_height / 2 - 1) * strideL + VREG_INT_COUNT),
	...
	if(dwt->parity == 0)
	{
		...
		if(CPUArch::SSE2() || CPUArch::AVX2())
		{
			...
			if(total_height > 1 && nb_cols == PLL_COLS_53)
			{
				...
				decompress_v_cas0_mcols_SSE2_OR_AVX2_53(dwt->mem, bandL, dwt->sn_full, strideL,
														bandH, dwt->dn_full, strideH, dest,
														strideDest);
				return;
			}
#endif
	...
	else
	{
		...
		if(CPUArch::SSE2() || CPUArch::AVX2())
		{
			...
			if(nb_cols == PLL_COLS_53)
			{
				/* Same as below general case, except that thanks to SSE2/AVX2 */
				/* we can efficiently process 8/16 columns in parallel */
				decompress_v_cas1_mcols_SSE2_OR_AVX2_53(dwt->mem, bandL, dwt->sn_full, strideL,
														bandH, dwt->dn_full, strideH, dest,
														strideDest);
				return;
			}
							 ...
							 uint8_t numres, ISparseCanvas* sa)
{
	...
	if(numres == 1U)
	{
		...
		bool ret = sa->read(0, synthesisWindow,
							tilec->getBuffer()->getHighestBufferResWindowREL()->getBuffer(), 1,
							tilec->getBuffer()->getHighestBufferResWindowREL()->stride, true);
		assert(ret);
		GRK_UNUSED(ret);
		return true;
	}
	...
	for(uint8_t resno = 1; resno < numres; resno++)
	{
		...
		bandWindowRect[BAND_ORIENT_LL] =
			*((grkRectU32*)tilec->getBuffer()->getBandWindowREL(resno, BAND_ORIENT_LL));
		bandWindowRect[BAND_ORIENT_HL] =
			*((grkRectU32*)tilec->getBuffer()->getBandWindowREL(resno, BAND_ORIENT_HL));
		bandWindowRect[BAND_ORIENT_LH] =
			*((grkRectU32*)tilec->getBuffer()->getBandWindowREL(resno, BAND_ORIENT_LH));
		bandWindowRect[BAND_ORIENT_HH] =
			*((grkRectU32*)tilec->getBuffer()->getBandWindowREL(resno, BAND_ORIENT_HH));
		...
		auto resWindowRect = *((grkRectU32*)tilec->getBuffer()->getBufferResWindowREL(resno));
		...
		splitWindowRect[SPLIT_L] =
			*((grkRectU32*)tilec->getBuffer()->getSplitWindowREL(resno, SPLIT_L));
		splitWindowRect[SPLIT_H] =
			*((grkRectU32*)tilec->getBuffer()->getSplitWindowREL(resno, SPLIT_H));
...
#endif
				if(!sa->write(resno, grkRectU32(resWindowRect.x0, j, resWindowRect.x1, j + height),
	...
	ret = sa->read(numres - 1, synthesisWindow,
				   tilec->getBuffer()->getHighestBufferResWindowREL()->getBuffer(), 1,
				   tilec->getBuffer()->getHighestBufferResWindowREL()->stride, true);
	assert(ret);
	GRK_UNUSED(ret);
...
#ifdef GRK_DEBUG_VALGRIND
	{
		...
		if(compno == debug_compno)
		{
			...
			{
				auto bufPtr = tilec->getBuffer()->getHighestBufferResWindowREL()->buf +
							  j * tilec->getBuffer()->getHighestBufferResWindowREL()->stride;
				for(uint32_t i = 0; i < tileSynthesisWindow.width(); i++)
				{
					...
					bufPtr += tilec->getBuffer()->getHighestBufferResWindowREL()->stride;
				}
			}
		}
	}
