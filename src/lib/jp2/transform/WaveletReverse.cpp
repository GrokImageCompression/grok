    ...
    if (dwt->parity == 0) {
    	...
    	if (CPUArch::SSE2() || CPUArch::AVX2() ) {
			...
			if (total_height > 1 && nb_cols == PLL_COLS_53) {
				...
				decompress_v_cas0_mcols_SSE2_OR_AVX2_53(dwt->mem, bandL,dwt->sn_full, strideL, bandH, dwt->dn_full, strideH, dest, strideDest);
				return;
			}
#endif
    	}
        if (total_height > 1) {
            for (uint32_t c = 0; c < nb_cols; c++, bandL++, bandH++,dest++)
                decompress_v_cas0_53(dwt->mem, bandL,dwt->sn_full, strideL,bandH,dwt->dn_full, strideH, dest, strideDest);
            return;
        }
    } else {
        if (total_height == 1) {
        ...
        if (CPUArch::SSE2() || CPUArch::AVX2() ) {
			...
			if (nb_cols == PLL_COLS_53) {
				/* Same as below general case, except that thanks to SSE2/AVX2 */
				/* we can efficiently process 8/16 columns in parallel */
				decompress_v_cas1_mcols_SSE2_OR_AVX2_53(dwt->mem, bandL,dwt->sn_full, strideL,bandH,dwt->dn_full, strideH, dest, strideDest);
				return;
			}
#endif
        }
		for (uint32_t c = 0; c < nb_cols; c++, bandL++,bandH++,dest++)
			decompress_v_cas1_53(dwt->mem, bandL,dwt->sn_full,strideL,bandH, dwt->dn_full, strideH, dest, strideDest);
    }
}
...
static bool decompress_tile_53( TileComponent* tilec, uint32_t numres){
    ...
    for (uint8_t res = 1; res < numres; ++res){
    	...
    	if (!decompress_h_mt_53(num_threads,
							...
							vert.sn_full,
							// LL
							tilec->getBuffer()->getBufferResWindowREL(res-1U)->data,
    	...
    	if (!decompress_v_mt_53(num_threads,
							...
							tilec->getBuffer()->getSplitWindowREL(res,SPLIT_H)->stride,
							// resolution buffer
							tilec->getBuffer()->getBufferResWindowREL(res)->data,
...
bool decompress_tile_97(TileComponent* GRK_RESTRICT tilec,uint32_t numres){
    ...
    for (uint8_t res = 1; res < numres; ++res) {
        ...
        if (!decompress_h_mt_97(num_threads,
							...
							vert.sn_full,
							// LL
							(float*) tilec->getBuffer()->getBufferResWindowREL(res-1U)->data,
							tilec->getBuffer()->getBufferResWindowREL(res-1U)->stride,
							// HL
        ...
        if (!decompress_v_mt_97(num_threads,
							...
							tilec->getBuffer()->getSplitWindowREL(res,SPLIT_H)->stride,
							// resolution window
							(float*) tilec->getBuffer()->getBufferResWindowREL(res)->data,
							tilec->getBuffer()->getBufferResWindowREL(res)->stride))
        	return false;
    }
    horiz.release();
...
template <typename T,
   ...
   bool decompress_partial_tile(TileComponent* GRK_RESTRICT tilec,
    ...
    if (numres == 1U) {
    	...
    	bool ret = sa->read(0,
    					synthesisWindow,
					   tilec->getBuffer()->getHighestBufferResWindowREL()->data,
                       1,
		...
		auto executor_h = [resno,compno,sa, resWindowRect, &decompressor](decompress_job<T, dwt_data<T>> *job){
			...
			(void)resno;
			 for (uint32_t j = job->min_j; j < job->max_j; j += HORIZ_PASS_HEIGHT) {
				...
				ss << "H interleave : compno = " << (uint32_t)compno  << ", resno= " << (uint32_t)(resno) << ", x begin = " << j << ", total samples = " << len;
...
#ifdef GRK_DEBUG_VALGRIND
				auto ptr = ((uint64_t)job->data.memL < (uint64_t)job->data.memH) ? job->data.memL : job->data.memH;
				ss <<  "V interleave: compno = " << (uint32_t)compno << ", resno= " << (uint32_t)(resno) << ", x begin = " << j << ", total samples = " << len;
				grk_memcheck_all<int32_t>((int32_t*)ptr, len, ss.str());
#endif
				job->data.memL  =  job->data.mem;
...
#ifdef GRK_DEBUG_VALGRIND
				ss.clear();
				ss << "V decompress: compno = " << (uint32_t)compno << ", resno= " << (uint32_t)(resno) << ", x begin = " << j << ", total samples = " << len;
				grk_memcheck_all<int32_t>((int32_t*)job->data.mem, len, ss.str());
#endif
				if (!sa->write(resno,
							grkRectU32(j,
	...
	if (compno == debug_compno) {
		for (uint32_t j = 0; j < tileSynthesisWindow.height();j++) {
			auto bufPtr = tilec->getBuffer()->getHighestBufferResWindowREL()->data + j * tilec->getBuffer()->getHighestBufferResWindowREL()->stride;
