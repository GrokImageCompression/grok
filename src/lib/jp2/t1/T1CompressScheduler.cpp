			...
			for (bandIndex = 0; bandIndex < res->numTileBandWindows; ++bandIndex) {
				...
				for (auto prc : band->precincts){
					...
					for (uint64_t cblkno = 0; cblkno < prc->getNumCblks();	++cblkno) {
						...
						block->tiledp = tilec->getBuffer()->getHighestBufferResWindowREL()->getBuffer() + (uint64_t) block->x +
											block->y * (uint64_t) tilec->getBuffer()->getHighestBufferResWindowREL()->stride;
