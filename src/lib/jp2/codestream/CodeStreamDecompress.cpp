		...
		for(uint16_t i = 0; i < numTilesToDecompress; ++i)
		{
			...
			auto exec = [this, processor, numTilesToDecompress, &numTilesDecompressed, &success] {
				if(success)
				{
					...
					{
						GRK_ERROR("Failed to decompress tile %u/%u", processor->m_tileIndex + 1,
...
bool CodeStreamDecompress::decompressTile()
{
	...
	if(!tileCache || !tileCache->processor->getImage())
	{
		...
		try
		{
			...
			{
				GRK_ERROR("Failed to decompress tile %u", tileProcessor->m_tileIndex + 1);
				goto cleanup;
			}
		}
		catch(DecodeUnknownMarkerAtEndOfTileException& e)
...
bool CodeStreamDecompress::read_mcc(uint8_t* p_header_data, uint16_t header_size)
{
	...
	for(i = 0; i < nb_collections; ++i)
	{
		...
		for(j = 0; j < mcc_record->m_nb_comps; ++j)
		{
			...
			if(tmp != j)
			{
				GRK_WARN("Cannot take in charge collections with indix shuffle");
				return true;
			}
		}
		...
		{
			GRK_WARN("Cannot take in charge collections without same number of indices");
			return true;
		}
