...
bool CodeStreamDecompress::parseTileHeaderMarkers(bool* canDecompress)
{
	...
	while(!decompressorState_.lastTilePartWasRead && (curr_marker_ != J2K_MS_EOC))
	{
		...
		while(curr_marker_ != J2K_MS_SOD)
		{
			...
			else if(marker_size < MARKER_ID_BYTES)
			{
			...
			marker_size =
				(uint16_t)(marker_size - MARKER_ID_BYTES); /* Subtract the size of the marker ID already read */
...
{
	uint32_t size_unk = MARKER_ID_BYTES;
	...
	while(true)
	{
		...
		{
			size_unk += MARKER_ID_BYTES;
			continue;
		}
		addMarker(unknownMarker, stream_->tell() - MARKER_ID_BYTES - size_unk, size_unk);
		auto marker_handler = get_marker_handler(curr_marker_);
...
{
	uint8_t data[MARKER_ID_BYTES];
	uint16_t marker;
	if(stream_->read(data, MARKER_ID_BYTES) != MARKER_ID_BYTES)
		return false;
	...
	if(codeStreamInfo)
	{
		...
		codeStreamInfo->setMainHeaderStart(stream_->tell() - MARKER_ID_BYTES);
		addMarker(J2K_MS_SOC, codeStreamInfo->getMainHeaderStart(), MARKER_ID_BYTES);
	}
	return true;
}
/**
