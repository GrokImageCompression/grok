...
static void decompress_help_display(void)
{
  ...
  fprintf(stdout, "\n");
  fprintf(stdout,
          "Number of layers. Set the maximum number of quality layers to decode. If there are\n");
  ...
  fprintf(stdout, "\n");
  fprintf(stdout,
          "Disable support for random access code stream markers if present : PLT,TLM or\n");
  fprintf(stdout, "PLM;\n");
  fprintf(stdout, "\n");
  fprintf(stdout,
  ...
  fprintf(stdout, "\n");
  fprintf(stdout, "```\n");
  fprintf(stdout, "1   Disable PLT marker if present\n");
  fprintf(stdout, "2   Disable TLM marker if present\n");
  fprintf(stdout, "4   Disable PLM marker if present\n");
  fprintf(stdout, "```\n");
  ...
  fprintf(stdout, "\n");
  fprintf(stdout, " `-L, --compression-level [compression level]`\n");
  fprintf(stdout, "\n");
  ...
  fprintf(stdout, "\n");
  fprintf(stdout, " `-W, --log-file [log file name]`\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "Log to file. File name will be set to `log file name`\n");
  fprintf(stdout, "\n");
...
void GrkDecompress::generateCompletion(const CLI::App& app)
{
  std::cout << "# Bash completion script for " << app.get_name() << "\n";
  std::cout << "_grk_decompress_completion() {\n";
  ...
  // Dynamic suggestions based on previous word
  std::cout << "    case $prev in\n";
  // File/directory options
  for(const auto* opt : {"-a", "--out-dir", "-g", "--plugin-path", "-i", "--in-file", "-o",
                         "--out-file", "-W", "--log-file", "-y", "--batch-src"})
  {
    std::cout << "        " << opt << ")\n";
    std::cout << "            COMPREPLY=( $(compgen -f -- \"$cur\") )\n";
    if(std::string(opt).find("dir") != std::string::npos ||
       std::string(opt).find("src") != std::string::npos)
    {
      std::cout << "            if [[ -z \"$cur\" || -d \"$cur\" ]]; then\n";
  ...
  std::cout << "    esac\n";
  std::cout << "}\n\n";
  std::cout << "complete -F _grk_decompress_completion grk_decompress\n";
}

GrkRC GrkDecompress::parseCommandLine(int argc, const char* argv[],
                                      DecompressInitParams* initParams)
{
  auto parameters = &initParams->parameters;
  auto inputFolder = &initParams->inputFolder;
  auto outFolder = &initParams->outFolder;
  auto pluginPath = initParams->pluginPath;
  ...
  auto layerOpt = cmd.add_option("-l,--layers", layer, "Number of layers to decompress")
                      ->check(CLI::Range((uint16_t)1, maxNumLayersJ2K));
  ...
  if(cmd["--generate-completion"]->as<bool>())
  {
    generateCompletion(cmd);
    return GrkRCUsage; // Assuming GrkRCSuccess exists; adjust as needed
  }
  bool useStdio =
  ...
  {
#ifdef _WIN32
    _putenv("GROK_DEBUG=0"); // Windows
#else
    setenv("GROK_DEBUG", "0", 1); // POSIX (Linux, macOS)
#endif
  }
...
GrkRC GrkDecompress::pluginMain(int argc, const char* argv[], DecompressInitParams* initParams)
{
  GrkRC rc = GrkRCFail;
...
int GrkDecompress::preProcess(grk_plugin_decompress_callback_info* info)
{
  ...
  switch(cod_format)
  {
    ...
    case GRK_FMT_PGX:
      ...
      break;
    case GRK_FMT_BMP:
      imageFormat = new BMPFormat<int32_t>();
      break;
    case GRK_FMT_TIF:
#ifdef GROK_HAVE_LIBTIFF
      imageFormat = new TIFFFormat<int32_t>();
#else
      ...
      break;
    case GRK_FMT_RAW:
      imageFormat = new RAWFormat<int32_t>(true);
      break;
    case GRK_FMT_RAWL:
    ...
    case GRK_FMT_PNG:
#ifdef GROK_HAVE_LIBPNG
      imageFormat = new PNGFormat<int32_t>();
#else
  ...
  if(!info->codec)
  {
    ...
    safe_strcpy(stream_params.file, infile);
  ...
  {
    spdlog::error("grk_decompress: null image");
    goto cleanup;
  }
  // 3. decompress
  if(info->tile)
  ...
  {
    bool adjustX =
        (info->image->x0 != (double)info->full_image_x0) && ((uint32_t)info->image->x0 & 1);
    bool adjustY =
        (info->image->y0 != (double)info->full_image_y0) && ((uint32_t)info->image->y0 & 1);
    if(adjustX || adjustY)
    {
  ...
  if(parameters->single_tile_decompress)
  {
    ...
    info->image = grk_decompress_get_tile_image(info->codec, parameters->tile_index, true);
  }
  // or, decompress all tiles
  else
  {
...
int GrkDecompress::postProcess(grk_plugin_decompress_callback_info* info)
{
  ...
  if(!image)
  {
    spdlog::error("postProcess: null image");
    goto cleanup;
  }
  infile = info->decompressor_parameters->infile[0] ? info->decompressor_parameters->infile
                                                    : info->input_file_name;
  outfile = info->decompressor_parameters->outfile[0] ? info->decompressor_parameters->outfile
                                                      : info->output_file_name;
  if(image->meta)
  {
...
int GrkDecompress::main(int argc, const char* argv[])
{
  int rc = EXIT_SUCCESS;
  uint32_t numDecompressed = 0;
  DecompressInitParams initParams;
