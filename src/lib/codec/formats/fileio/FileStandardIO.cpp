...
FileStandardIO::FileStandardIO() : fileHandle_(nullptr) {}

bool FileStandardIO::open(const std::string& fileName, const std::string& mode)
{
...
bool FileStandardIO::close(void)
{
  bool rc = true;
...
uint64_t FileStandardIO::write(uint8_t* buf, [[maybe_unused]] uint64_t offset, size_t len,
...
uint64_t FileStandardIO::write(GrkIOBuf buffer)
{
  assert(fileHandle_ != stdin);
...
bool FileStandardIO::read(uint8_t* buf, size_t len)
{
...
uint64_t FileStandardIO::seek(int64_t off, int whence)
{
  assert(fileHandle_ != stdin);
#ifndef _MSC_VER
...
FILE* FileStandardIO::getFileHandle()
{
  return fileHandle_;
}
