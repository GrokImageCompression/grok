...
template<typename T>
grk_image* pgxtoimage(const char* filename, const grk_cparameters* parameters)
{
  ...
  for(uint32_t j = 0; j < h; ++j)
  {
    for(uint32_t k = 0; k < w; ++k)
    {
      ...
      if(prec < 8)
      {
        v = sign ? sign_extend((T)readchar<int8_t>(f), shift) : readchar<int8_t>(f);
      }
      else
      {
        ...
        {
          if(!image->comps->sgnd)
            v = (T)readchar<uint8_t>(f);
          else
            v = (T)readchar<int8_t>(f);
        }
        else
        {
          if(!image->comps->sgnd)
            v = (T)readshort<uint16_t>(f, bigendian);
          else
            v = (T)readshort<int16_t>(f, bigendian);
        }
      }
      dest[index++] = v;
    }
...
bool PGXFormat<T>::encodePixels(void)
{
  ...
  for(uint16_t compno = 0; compno < image_->numcomps; compno++)
  {
    ...
    if(!fileIO_->open(fileOut.c_str(), "wb"))
    {
    ...
    fprintf(fileIO_->getFileHandle(), "PG ML %c %u %u %u\n", comp->sgnd ? '-' : '+', comp->prec, w,
            h);
    ...
    if(nbytes == 1)
    {
      ...
      if(outCount)
      {
        size_t res = fwrite(buf, sizeof(uint8_t), outCount, fileIO_->getFileHandle());
        if(res != outCount)
        {
          spdlog::error("failed to write bytes for {}", fileOut);
          goto beach;
    ...
    else
    {
      ...
      if(outCount)
      {
        size_t res = fwrite(buf, sizeof(uint16_t), outCount, fileIO_->getFileHandle());
        if(res != outCount)
        {
          spdlog::error("failed to write bytes for {}", fileOut);
          goto beach;
