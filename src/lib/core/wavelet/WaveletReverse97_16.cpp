...
namespace HWY_NAMESPACE
{
  ...
  /**************************************************************************
   *  Scalar Horizontal 1D 16-bit 9/7 Synthesis
   *
   *  Processes one row of the 2D DWT: loads subband data from the low-pass
   *  (L) and high-pass (H) band buffers, interleaves with K/invK scaling,
   *  applies the four synthesis lifting steps, and writes to destination.
   *
   *  BOUNDARY EXTENSION: At the edges of the signal, missing neighbors are
   *  replaced by symmetric reflection (whole-sample symmetry), as required
   *  by [T.800] Annex F.3.  For a missing left neighbor, the right neighbor
   *  is used, and vice versa.
   *
   *  @param scratch  Scratch buffer (must hold ≥ width samples)
   *  @param width    Output width in samples (= sn + dn)
   *  @param bandL    Low-pass subband samples (sn values, contiguous)
   *  @param bandH    High-pass subband samples (dn values, contiguous)
   *  @param dest     Destination row buffer (width samples, contiguous)
   *  @param parity   0 if the first output sample is even (from L band),
   *                  1 if the first output sample is odd (from H band).
   *                  Determined by the tile grid offset: parity = x0 & 1.
   *  @param sn       Number of low-pass samples
   *  @param dn       Number of high-pass samples
   **************************************************************************/
  static void scalar_h_synth_16_97(int16_t* scratch, uint32_t width, const int16_t* bandL,
                                   const int16_t* bandH, int16_t* dest, uint32_t parity,
                                   [[maybe_unused]] uint32_t sn, [[maybe_unused]] uint32_t dn)
  {
    ...
    if(width == 1)
    {
      // Single-sample case: just apply the appropriate gain factor.
      // No lifting steps needed (no neighbors exist).
      if(parity == 0)
        dest[0] = apply_K_scale(bandL[0]);
      else
        dest[0] = apply_invK_scale(bandH[0]);
      return;
    }

    /* ------------------------------------------------------------------ */
    /*  Phase 1: Interleave subband samples with K / (2/K) gain scaling   */
    /*                                                                    */
    /*  After interleaving, scratch[i] contains:                          */
    /*    - Even positions (relative to parity): L samples scaled by K    */
    /*    - Odd  positions (relative to parity): H samples scaled by 2/K  */
    /* ------------------------------------------------------------------ */
    uint32_t lIdx = 0, hIdx = 0;
    for(uint32_t i = 0; i < width; ++i)
    {
      if((i + parity) % 2 == 0)
        scratch[i] = apply_K_scale(bandL[lIdx++]);
      else
        scratch[i] = apply_invK_scale(bandH[hIdx++]);
    }

    /* ------------------------------------------------------------------ */
    /*  Phase 2: Four synthesis lifting steps (reverse analysis order)     */
    /*                                                                    */
    /*  Step numbering follows the ANALYSIS convention:                    */
    /*    Step 3 (δ) → Step 2 (γ) → Step 1 (β) → Step 0 (α)             */
    /*  which is the REVERSE of analysis step 0 → 1 → 2 → 3.            */
    /* ------------------------------------------------------------------ */

    // Step 3 — Undo analysis update δ: S[n] -= δ × (D[n-1] + D[n])
    // Subtracts because δ is positive (analysis added, synthesis subtracts).
    for(uint32_t i = parity; i < width; i += 2)
    {
      int16_t d_prev = (i > 0) ? scratch[i - 1] : scratch[i + 1];
      int16_t d_next = (i + 1 < width) ? scratch[i + 1] : scratch[i - 1];
      scratch[i] = (int16_t)(scratch[i] - mf15((int16_t)(d_prev + d_next), synth_coeff_3));
    }

    // Step 2 — Undo analysis predict γ: D[n] -= γ × (S[n] + S[n+1])
    // Subtracts because γ is positive.
    for(uint32_t i = 1 - parity; i < width; i += 2)
    {
      int16_t s_prev = (i > 0) ? scratch[i - 1] : scratch[i + 1];
      int16_t s_next = (i + 1 < width) ? scratch[i + 1] : scratch[i - 1];
      scratch[i] = (int16_t)(scratch[i] - mf15((int16_t)(s_prev + s_next), synth_coeff_2));
    }

    // Step 1 — Undo analysis update β: S[n] += |β| × (D[n-1] + D[n])
    // Adds because β is negative (analysis subtracted, synthesis adds back).
    // Uses MULTIPLY-FIRST with ×8 amplification and >>3 correction
    // (see Section 6, precision boost strategy).
    for(uint32_t i = parity; i < width; i += 2)
    {
      int16_t d_prev = (i > 0) ? scratch[i - 1] : scratch[i + 1];
      int16_t d_next = (i + 1 < width) ? scratch[i + 1] : scratch[i - 1];
    ...
    // Step 0 — Undo analysis predict α: D[n] += |α| × (S[n] + S[n+1])
    // Adds because α is negative. Uses additive decomposition:
    //   D[n] += sum + MulFixedPoint15(sum, frac)
    // where sum = S[n] + S[n+1] and frac = |α| − 1 = 0.586 (see Section 6).
    for(uint32_t i = 1 - parity; i < width; i += 2)
    {
      int16_t s_prev = (i > 0) ? scratch[i - 1] : scratch[i + 1];
      int16_t s_next = (i + 1 < width) ? scratch[i + 1] : scratch[i - 1];
