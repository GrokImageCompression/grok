...
bool WaveletReverse::decompress_h_97(uint8_t res, uint32_t num_workers, size_t dataLength,
                                     ...
                                     grk_buf2d_simple<float> winDest)
{
  ...
  else
  {
    ...
    {
      grklog.warn("Missing image component flow");
      return true;
    }
...
bool WaveletReverse::decompress_v_97(uint8_t res, uint32_t num_workers, size_t dataLength,
...
bool WaveletReverse::decompress_tile_97(void)
{
  ...
  {
    grklog.error("decompress_tile_97: out of memory");
    return false;
  }
  vertF_.mem = horizF_.mem;
  uint32_t num_workers = (uint32_t)ExecSingleton::get().num_workers();
  for(uint8_t res = 1; res < numres_; ++res)
  {
    ...
    if(!decompress_h_97(res, num_workers, dataLength, horizF_, vertF_.sn_full,
                        buf->getResWindowBufferSimpleF(res - 1U),
                        buf->getBandWindowBufferPaddedSimpleF(res, BAND_ORIENT_HL), winSplitL))
      return false;
    if(!decompress_h_97(res, num_workers, dataLength, horizF_, resHeight - vertF_.sn_full,
    ...
    if(!decompress_v_97(res, num_workers, dataLength, vertF_, resWidth, resHeight, winSplitL,
                                     ...
                                     uint32_t resHeight, size_t dataLength)
{
  ...
  for(uint32_t orient = 0; orient < 2; ++orient)
  {
    ...
    if(num_workers > 1)
      numTasks[orient] = height[orient] < num_workers ? height[orient] : num_workers;
  }
  for(uint32_t orient = 0; orient < 2; ++orient)
  {
    if(height[orient] == 0)
                                     ...
                                     uint32_t resWidth, size_t dataLength)
{
  ...
  else
  {
    auto imageComponentFlow = scheduler_->getImageComponentFlow(compno_);
    auto resFlow = imageComponentFlow->getResFlow(res - 1);
    const uint32_t numTasks = resWidth < num_workers ? resWidth : num_workers;
