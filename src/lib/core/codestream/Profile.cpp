...
bool Profile::isImfCompliant(grk_cparameters* parameters, GrkImage* image)
{
  ...
  if((int)sublevel > m)
  {
    grklog.warn("JPEG 2000 IMF profile: invalid sub-level %u", sublevel);
    ret = false;
  }
  ...
  if(parameters->tile_size_on)
  {
    ...
    else
    {
      ...
      {
        grklog.warn("IMF 2K_R/4K_R/8K_R single/multiple tile profiles "
...
void Profile::setCinemaParams(grk_cparameters* parameters, GrkImage* image)
{
  ...
  if(parameters->numlayers > 1)
  {
    grklog.warn("JPEG 2000 profiles 3 and 4 (2k and 4k digital cinema) require:\n"
