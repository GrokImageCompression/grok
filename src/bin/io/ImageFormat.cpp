...
ImageFormat::ImageFormat(bool flushOnClose, uint8_t* header, size_t headerLength)
	: header_(header), headerLength_(headerLength), encodeState_(IMAGE_FORMAT_UNENCODED),
	  serializer_(UINT_MAX, flushOnClose), imageStripper_(nullptr), concurrency_(0),
	  workerSerializers_(nullptr), numPixelWrites_(0), maxPixelWrites_(0)
{}
...
void ImageFormat::init(uint32_t width, uint32_t height, uint16_t numcomps, uint64_t packedRowBytes,
					   uint32_t nominalStripHeight, bool chunked)
{
	imageStripper_ =
		new ImageStripper(width, height, numcomps, packedRowBytes, nominalStripHeight,
						  headerLength_, WRTSIZE, chunked ? serializer_.getPool() : nullptr);

	maxPixelWrites_ = chunked ? imageStripper_->numUniqueChunks() : imageStripper_->numStrips();
}
bool ImageFormat::encodeInit(std::string filename, bool direct, uint32_t concurrency, bool asynch)
{
	filename_ = filename;
...
StripChunkArray* ImageFormat::getStripChunkArray(uint32_t threadId, uint32_t strip)
{
	...
	return imageStripper_->getStrip(strip)->getStripChunkArray(pool, strip == 0 ? header_ : nullptr,
...
bool ImageFormat::encodePixels(uint32_t threadId, IOBuf** buffers, uint32_t numBuffers)
{
	assert(numBuffers);
