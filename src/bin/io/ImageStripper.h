...
struct ChunkInfo
{
	...
	ChunkInfo(bool isFirstStrip, bool isFinalStrip, uint64_t logicalOffset, uint64_t logicalLen,
			  uint64_t logicalOffsetPrev, uint64_t logicalLenPrev, uint64_t headerSize,
			  uint64_t writeSize)
		: isFirstStrip_(isFirstStrip), isFinalStrip_(isFinalStrip), writeSize_(writeSize),
		  headerSize_(headerSize), pool_(nullptr)
	{
		if(!writeSize_)
			return;
		...
		first_.x1_ =
			(isFirstStrip_ ? 0 : lastBegin(logicalOffsetPrev, logicalLenPrev)) + writeSize_;
	...
	ChunkInfo(const ChunkInfo& rhs)
		: first_(rhs.first_), last_(rhs.last_), isFirstStrip_(rhs.isFirstStrip_),
		  isFinalStrip_(rhs.isFinalStrip_), writeSize_(rhs.writeSize_),
		  headerSize_(rhs.headerSize_), pool_(nullptr)
	{}
	uint64_t len()
	{
...
struct IOChunk : public RefCounted
{
	IOChunk(uint64_t offset, uint64_t len, uint64_t allocLen, IBufferPool* pool)
		: offset_(offset), len_(len), allocLen_(allocLen), buf_(nullptr), acquireCount_(0),
		  shareCount_(1)
	{
		if(pool)
		{
...
{
	StripChunk(IOChunk* ioChunk, uint64_t writeableOffset, uint64_t writeableLen)
		: writeableOffset_(writeableOffset), writeableLen_(writeableLen), ioChunk_(ioChunk)
	{
		// we may need to extend a shared ioChunk_'s length
		if(ioChunk_->isShared() && ioChunk_->offset_ + ioChunk_->len_ == writeableOffset)
...
struct StripChunkArray
{
	StripChunkArray(StripChunk** chunks, IOBuf** buffers, uint64_t numBuffers, IBufferPool* pool)
		: ioBufs_(buffers), stripChunks_(chunks), numBuffers_(numBuffers), pool_(pool)
	{
...
struct Strip
{
	Strip(uint64_t offset, uint64_t len, Strip* neighbour)
		: logicalOffset_(offset), logicalLen_(len), stripChunks_(nullptr), numChunks_(0),
	...
	void generateChunks(ChunkInfo chunkInfo, IBufferPool* pool)
	{
		...
		if(numChunks_ == 1)
		{
			...
			{
				ioChunk = new IOChunk(0, chunkInfo.first_.x1_, chunkInfo.writeSize_,
		...
		{
			uint64_t off =
				(chunkInfo.first_.x1_ - chunkInfo_.writeSize_) + i * chunkInfo_.writeSize_;
			bool lastChunkOfAll = chunkInfo.isFinalStrip_ && (i == numChunks_ - 1);
			uint64_t len = lastChunkOfAll ? (chunkInfo_.last_.len()) : chunkInfo_.writeSize_;
			uint64_t writeableOffset = 0;
			uint64_t writeableLen = len;
			bool sharedLastChunk = false;
			bool firstSeam = (i == 0) && chunkInfo.hasFirstSeam();
			bool lastSeam = (i == numChunks_ - 1) && !lastChunkOfAll && chunkInfo.hasLastSeam();
			if(firstSeam)
			{
			...
			stripChunks_[i] = new StripChunk(ioChunk, writeableOffset, writeableLen);
		}
		...
		{
			writeableEnd = stripChunks_[0]->offset() + stripChunks_[0]->writeableOffset_ +
						   stripChunks_[0]->writeableLen_;
		}
		assert(writeableEnd == chunkInfo_.last_.x1_);

		uint64_t writeableBegin = stripChunks_[0]->offset() + stripChunks_[0]->writeableOffset_;
		(void)writeableBegin;
		assert(writeableBegin ==
			   chunkInfo_.first_.x0_ + (chunkInfo_.isFirstStrip_ ? chunkInfo_.headerSize_ : 0));
...
{
	ImageStripper(uint32_t width, uint32_t height, uint16_t numcomps, uint64_t packedRowBytes,
				  uint32_t nominalStripHeight, uint64_t headerSize, uint64_t writeSize,
				  IBufferPool* pool)
		: width_(width), height_(height), numcomps_(numcomps),
		  nominalStripHeight_(nominalStripHeight),
		  numStrips_(nominalStripHeight ? (height + nominalStripHeight - 1) / nominalStripHeight
										: 0),
		  ...
		  finalStripHeight_((nominalStripHeight && (height % nominalStripHeight != 0))
								...
								: nominalStripHeight),
		  headerSize_(headerSize), writeSize_(writeSize), finalStrip_(numStrips_ - 1),
		  strips_(new Strip*[numStrips_])
	{
		for(uint32_t i = 0; i < numStrips_; ++i)
		{
			auto neighbour = (i > 0) ? strips_[i - 1] : nullptr;
			strips_[i] = new Strip(i * nominalStripHeight_ * packedRowBytes_,
								   stripHeight(i) * packedRowBytes_, neighbour);
			if(pool)
	...
	{
		return ChunkInfo(strip == 0, strip == finalStrip_, strips_[strip]->logicalOffset_,
						 strips_[strip]->logicalLen_,
						 strip == 0 ? 0 : strips_[strip - 1]->logicalOffset_,
						 strip == 0 ? 0 : strips_[strip - 1]->logicalLen_, headerSize_, writeSize_);
	}
	uint32_t width_;
	uint32_t height_;
	uint16_t numcomps_;
