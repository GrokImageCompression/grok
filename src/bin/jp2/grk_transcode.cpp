...
static void transcodeHelp(void)
{
	fprintf(stdout,
...
void GrkTranscode::printTiming(uint32_t num_images, std::chrono::duration<double> elapsed)
{
	if(!num_images)
		return;
...
bool GrkTranscode::parsePrecision(const char* option, grk_decompress_parameters* parameters)
{
...
int GrkTranscode::loadImages(grk_dircnt* dirptr, char* imgdirpath)
{
	DIR* dir;
...
char GrkTranscode::nextFile(std::string inputFile, grk_img_fol* inputFolder, grk_img_fol* outFolder,
 ...
 */
/* -------------------------------------------------------------------------- */
int GrkTranscode::parseCommandLine(int argc, char** argv, TranscodeInitParams* initParams)
{
...
void GrkTranscode::setDefaultParams(grk_decompress_parameters* parameters)
{
	if(parameters)
	{
...
void GrkTranscode::destoryParams(grk_decompress_parameters* parameters)
{
	if(parameters)
	{
		if(parameters->precision)
...
int GrkTranscode::decompress(const char* fileName, TranscodeInitParams* initParams)
{
...
int GrkTranscode::pluginMain(int argc, char** argv, TranscodeInitParams* initParams)
{
...
int GrkTranscode::preProcess(grk_plugin_decompress_callback_info* info)
{
	if(!info)
		return 1;
	bool failed = true;
 ...
 */
int GrkTranscode::postProcess(grk_plugin_decompress_callback_info* info)
{
	if(!info)
		return -1;
	...
	try
	{
		...
		for(uint32_t i = 0; i < initParams.parameters.repeats; ++i)
		{
			...
			else
			{
				...
				if(!dir)
				{
					spdlog::error("Could not open Folder {}", initParams.inputFolder.imgdirpath);
					rc = EXIT_FAILURE;
					goto cleanup;
				}
...
GrkTranscode::GrkTranscode() : storeToDisk(true), imageFormat(nullptr) {}
