...
// a region decode of an irreversible 9/7 image at 8 bits or less returns int16 samples,
// through the windowed Q1.15 partial wavelet, with the Q-format downshift and the dc
// level shift fused into its final read. both the region and the whole image run the
// same fixed-point kernel and the FILTER_WIDTH padding makes the window reconstruction
// exact, so the samples have to match the same rectangle cropped out of a whole image
// decode, with no tolerance.
//
...
{
  std::string name;
  std::string path = std::string("region_decompress_97_int16_") +
