...
// T.800 A.4.2 lets the last tile-part of a codestream carry Psot=0, meaning it
// runs to the EOC. The fast path has to walk such a chain and decode, sample
// for sample, what the classic pipeline decodes. Takes one reversible
// codestream.
...
bool sameImage(const Decoded& classic, const Decoded& mercury)
{
  ...
  {
    fprintf(stderr, "geometry mismatch: %u,%u..%u,%u/%u vs %u,%u..%u,%u/%u\n", classic.x0,
            classic.y0, classic.x1, classic.y1, classic.numcomps, mercury.x0, mercury.y0,
            mercury.x1, mercury.y1, mercury.numcomps);
    return false;
  }
  for(uint16_t c = 0; c < classic.numcomps; ++c)
  {
    ...
    {
      fprintf(stderr,
              "component %u layout mismatch: %ux%u prec %u sgnd %d vs "
              "%ux%u prec %u sgnd %d\n",
              c, a.w, a.h, a.prec, (int)a.sgnd, b.w, b.h, b.prec, (int)b.sgnd);
      return false;
    }
    for(size_t i = 0; i < a.samples.size(); ++i)
    {
      ...
      {
        fprintf(stderr, "component %u sample %zu (x %zu, y %zu) differs: %d vs %d\n", c, i, i % a.w,
                i / a.w, a.samples[i], b.samples[i]);
        return false;
      }
    }
  }
...
int main(int argc, char** argv)
{
  ...
  Decoded classic;
  Decoded mercury;
  if(!decode(path, false, classic) || !decode(path, true, mercury))
    result = 1;
  else
  {
    ...
    {
      fprintf(stderr,
              "fell back to the classic pipeline: no \"%s\" in the log.\ncaptured log:\n%s\n",
              MERCURY_SUCCESS_MARKER, log.c_str());
      result = 1;
    }
    else if(log.find(MERCURY_PLAN_REJECTED_MARKER) != std::string::npos)
    {
      fprintf(stderr,
              "mercury was launched and rejected the plan itself; the eligibility "
              "check should have bailed first.\ncaptured log:\n%s\n",
              log.c_str());
      result = 1;
    }
    else if(!sameImage(classic, mercury))
      result = 1;
  }
