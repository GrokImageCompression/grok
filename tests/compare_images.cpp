...
static void compare_images_help_display(void) {
  ...
  fprintf(
      stdout,
      "  -n \t REQUIRED \t number of components in the image (used to generate "
      "correct filename; not used when both input files are TIF)\n");
  fprintf(stdout,
          " -d \t OPTIONAL \t indicates that utility will run as "
  ...
  fprintf(stdout,
          "  -R \t OPTIONAL \t Sub-region of base image to compare with test "
...
static char *createMultiComponentsFilename(std::string inFilename,
  ...
  GRK_SUPPORTED_FILE_FMT decod_format =
      grk::get_file_format(inFilename.c_str());
...
static grk_image *readImageFromFilePPM(const char *filename,
  ...
  dest = grk_image_new(nullptr,(uint16_t)nbFilenamePGX, src_param, GRK_CLRSPC_UNKNOWN,
                       true);
  if (!dest || !dest->comps)
    goto cleanup;
  for (fileno = 0; fileno < nbFilenamePGX; fileno++) {
...
static grk_image *readImageFromFilePGX(const char *filename,
                                       uint16_t nbFilenamePGX,
                                       const char *separator) {
  uint16_t fileno;
  grk_image *src = nullptr;
  ...
  dest = grk_image_new(nullptr,nbFilenamePGX, dest_param, GRK_CLRSPC_UNKNOWN, true);
  if (!dest || !dest->comps)
    goto cleanup;
  for (fileno = 0; fileno < nbFilenamePGX; fileno++) {
...
static int imageToPNG(const grk_image *src, const char *filename,
  ...
  auto dest = grk_image_new(nullptr,1u, &dest_param, GRK_CLRSPC_GRAY, true);
...
static int parse_cmdline_cmp(int argc, char **argv,
  ...
  try {
    ...
    TCLAP::ValueArg<std::string> testImageArg("t", "Test", "Test Image", true,
                                              "", "string", cmd);
    TCLAP::ValueArg<uint32_t> numComponentsArg(
        "n", "NumComponents", "Number of components", true, 1, "uint32_t", cmd);

    TCLAP::ValueArg<std::string> mseArg("m", "MSE", "Mean Square Energy", false,
                                        "", "string", cmd);
    TCLAP::ValueArg<std::string> psnrArg(
        "p", "PSNR", "Peak Signal To Noise Ratio", false, "", "string", cmd);

    TCLAP::SwitchArg nonRegressionArg("d", "NonRegression", "Non regression",
                                      cmd);
    TCLAP::ValueArg<std::string> separatorArg("s", "Separator", "Separator",
                                              false, "", "string", cmd);

    TCLAP::ValueArg<std::string> regionArg(
        "R", "SubRegion", "Sub region to compare", false, "", "string", cmd);
    ...
    if (flagM && flagP) {
      ...
      param->tabPEAKvalues =
          parseToleranceValues(PEAKlistvalues, param->nbcomp);
    ...
    if ((!param->nr_flag) && (!flagP || !flagM)) {
      spdlog::info(
          "Non-regression flag must be set if"
...
int main(int argc, char **argv) {
  ...
  for (compno = 0; compno < imageBase->numcomps; compno++) {
    ...
    if (baseComp->sgnd != testComp->sgnd) {
      spdlog::error("sign mismatch [comp {}] ({} != {})", compno,
                    baseComp->sgnd, testComp->sgnd);
      goto cleanup;
    }
    ...
    if (inParam.regionSet) {
      if (testComp->w != inParam.region[2] - inParam.region[0]) {
        spdlog::error(
            "test image component width {} doesn't match region width {}",
            testComp->w, inParam.region[2] - inParam.region[0]);
        goto cleanup;
      }
      if (testComp->h != inParam.region[3] - inParam.region[1]) {
        spdlog::error(
            "test image component height {} doesn't match region height {}",
      ...
      if (baseComp->h != testComp->h) {
        spdlog::error("height mismatch [comp {}] ({} != {})", compno,
                      baseComp->h, testComp->h);
        goto cleanup;
      }

      if (baseComp->w != testComp->w) {
        spdlog::error("width mismatch [comp {}] ({} != {})", compno,
                      baseComp->w, testComp->w);
        goto cleanup;
      }
    }

    if (baseComp->prec != testComp->prec) {
      spdlog::error("precision mismatch [comp {}] ({} != {})", compno,
                    baseComp->prec, testComp->prec);
      goto cleanup;
    }
  ...
  imageDiff = grk_image_new(nullptr,imageBase->numcomps, param_image_diff,
  ...
  for (compno = 0; compno < imageDiff->numcomps; compno++) {
    ...
    if (!inParam.nr_flag && (inParam.tabMSEvalues != nullptr) &&
      ...
      if (nbPixelDiff != 0) {
        ...
        spdlog::info(
            "<DartMeasurement name=\"NumberOfPixelsWithDifferences_{}\" "
