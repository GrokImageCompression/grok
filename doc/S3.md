...
All environment variables are compatible with GDAL's `/vsis3/` virtual filesystem,
so existing GDAL-based workflows can switch to Grok without changing configuration.
...
|----------|---------|-------------|
| `CPL_AWS_CREDENTIALS_FILE` | `~/.aws/credentials` | Override credentials file path |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CPL_AWS_SSO_ENDPOINT` | `portal.sso.<region>.amazonaws.com` | Override SSO endpoint |
...
| `CPL_AWS_WEB_IDENTITY_ENABLE` | `YES` (default) / `NO` — disable this method |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CPL_AWS_EC2_API_ROOT_URL` | `http://169.254.169.254` | Override metadata endpoint |
| `CPL_AWS_AUTODETECT_EC2_DISABLE` | `NO` | Set to `YES` to skip EC2 metadata |
...
| `CPL_AWS_STS_ROOT_URL` | *(auto)* | Override STS endpoint entirely |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CPL_VSIL_CURL_ALLOW_INSECURE` | `NO` | Disable SSL certificate verification |
| `GDAL_HTTP_UNSAFESSL` | `NO` | Disable SSL verification (inherited from CurlFetcher) |

### Timeouts

| Variable | Default | Description |
|----------|---------|-------------|
| `CPL_VSIL_CURL_TIMEOUT` | *(none)* | Request timeout in seconds |

### Caching

| Variable | Default | Description |
|----------|---------|-------------|
| `CPL_VSIL_CURL_CACHE_SIZE` | *(none)* | Curl buffer size in bytes |

### Connection Reuse

| Variable | Description |
|----------|-------------|
| `CPL_VSIL_CURL_NON_CACHED` | Colon-separated list of prefixes to disable connection reuse for (e.g. `/vsis3/`) |
...
| `CPL_VSIL_CURL_PROXYUSERPWD` | Proxy credentials (`user:password`) |
| `CPL_VSIL_CURL_PROXYAUTH` | Proxy auth type (any value enables `CURLAUTH_ANY`) |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CPL_AWS_ROOT_DIR` | `~/.aws` | Override AWS config root directory |
